...
import os
import sys
from spline_based_search_tree import cubic_spline
from spline_based_search_tree import quartic_spline 
from spline_based_search_tree import Quintic_spline 

import matplotlib.pyplot as plt
import numpy as np
import sc627_project.predictive_constraints as env
import sc627_project.draw as draw
...
def sampling_paths_for_Cruising(l0, l0_v, l0_a, s0, s0_v, s0_a, ref_path):
    ...
    for s1_v in np.arange(C.TARGET_SPEED * 0.6, C.TARGET_SPEED * 1.4, C.TARGET_SPEED * 0.2):
        ...
        for t1 in np.arange(4.5, 5.5, 0.2):
            path_pre = Path()
            path_lon = quartic_spline.QuarticPolynomial(s0, s0_v, s0_a, s1_v, 0.0, t1)
            ...
            for l1 in np.arange(-C.ROAD_WIDTH, C.ROAD_WIDTH, C.ROAD_SAMPLE_STEP):
                ...
                path_lat = Quintic_spline.QuinticPolynomial(l0, l0_v, l0_a, l1, 0.0, 0.0, t1)
...
def sampling_paths_for_Stopping(l0, l0_v, l0_a, s0, s0_v, s0_a, ref_path):
    ...
    for s1_v in [-1.0, 0.0, 1.0, 2.0]:
        ...
        for t1 in np.arange(1.0, 16.0, 1.0):
            path_pre = Path()
            path_lon = Quintic_spline.QuinticPolynomial(s0, s0_v, s0_a, 55.0, s1_v, 0.0, t1)
...
def main_Crusing():
    ...
    rx, ry, ref_path = get_reference_line(wx, wy)
